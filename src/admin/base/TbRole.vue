  ...
  <div>
    <b-form>
      <b-breadcrumb>
        ...
        <b-breadcrumb-item>角色</b-breadcrumb-item>
      </b-breadcrumb>
      <hr class="mt-0"/>
      <b-row>
        <b-col cols="5">
...
const fields = [
  ...
  {key: "roleCode", label: "角色编码", sortable: false},
  {key: "status", label: "状态", sortable: true},
  {key: "createTime", label: "key名称", sortable: true},
  {key: "updateTime", label: "value值", sortable: true},
  {key: "action", label: "操作"}
];
...
export default {
  ...
  methods: {
    ...
    queryById:function(id){
      this.$http.post("/api/tbRole/detail",{id:id}).then(resp=>{
    ...
    add: function () {
      ...
      this.$http.post("/api/tbRole/add", this.addData).then(resp => {
    ...
    del: function (id) {
      this.$bvModal.msgBoxConfirm("确定删除吗?").then(flag=>{
        if(flag){
          this.$http.post("/api/tbRole/delete", {id:id}).then(()=>{
            this.list();
          })
        }
      })
    ...
    update: function () {
      ...
      this.$http.post("/api/tbRole/update", this.updateData).then(resp => {
    ...
    //查
    list: function () {
      this.$http.post("/api/tbRole/pageList", this.search).then(resp => {
    ...
    batchDelete:function (){
      this.$bvModal.msgBoxConfirm("确定删除吗",{title:"删除提示",headerTextVariant:"danger"}).then(flag=>{
        if(flag){
          ...
          this.$http.post("/api/tbRole/batchDelete",filterIdsArr).then(()=>{
            this.list();
          })
        }
      })
