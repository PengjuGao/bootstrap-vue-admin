  ...
  <div>
    <b-form>
      <b-breadcrumb>
        ...
        <b-breadcrumb-item>菜单</b-breadcrumb-item>
      </b-breadcrumb>
      <hr class="mt-0"/>
      <b-row>
        <b-col cols="3">
...
export default {
  ...
  methods: {
    ...
    queryById:function(id){
      this.$http.post("/api/tbMenu/detail",{id:id}).then(resp=>{
    ...
    add: function () {
      ...
      this.$http.post("/api/tbMenu/add", this.addData).then(resp => {
    ...
    del: function (id) {
      this.$bvModal.msgBoxConfirm("确定删除吗?").then(flag=>{
        if(flag){
          this.$http.post("/api/tbMenu/delete", {id:id}).then(()=>{
            this.list();
          })
        }
      })
    ...
    update: function () {
      ...
      this.$http.post("/api/tbMenu/update", this.updateData).then(resp => {
    ...
    //查
    list: function () {
      this.$http.post("/api/tbMenu/pageList", this.search).then(resp => {
    ...
    batchDelete:function (){
      this.$bvModal.msgBoxConfirm("确定删除吗",{title:"删除提示",headerTextVariant:"danger"}).then(flag=>{
        if(flag){
          ...
          this.$http.post("/api/tbMenu/batchDelete",filterIdsArr).then(()=>{
            this.list();
          })
        }
      })
